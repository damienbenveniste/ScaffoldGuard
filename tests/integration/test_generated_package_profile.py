"""Integration tests for `agent-safe init` package generation."""
...
def test_init_codex_generates_valid_package_tree(
    ...
    assert "Created agent-safe Python project: demo" in result.output
...
def test_check_passes_in_fresh_generated_project(
    ...
    """A freshly generated all-adapter project passes `agent-safe check`."""
    monkeypatch.chdir(tmp_path)
    ...
    assert "agent-safe check: ok" in check_result.output
...
def test_check_text_output_reports_failure_locations(
    ...
    assert "agent-safe check: failed" in check_result.output
...
def test_init_dry_run_creates_no_files(
    ...
    assert result.exit_code == SUCCESS, result.output
    assert "Planned agent-safe Python project: demo" in result.output
...
def test_init_force_overwrites_generated_files_only(
    ...
    assert result.exit_code == SUCCESS, result.output
    assert "Generated by `agent-safe-python`" in (project_dir / "README.md").read_text(
        encoding="utf-8"
    )
