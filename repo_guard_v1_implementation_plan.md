# V1 Implementation Plan: `repo-guard`
...
**Working PyPI package name:** `agent-ready-python`
...
```bash
uvx agent-ready-python init my_project --agent codex
uvx agent-ready-python init my_project --agent claude
uvx agent-ready-python init my_project --agent cursor
...
```bash
repo-guard init NAME [options]
repo-guard check [options]
repo-guard inspect-diff [options]
repo-guard validate [options]
repo-guard compile-rules [options]
repo-guard doctor [options]
repo-guard version
```
...
packages = ["src/repo_guard"]

[tool.hatch.build.targets.wheel.force-include]
"src/repo_guard/templates" = "repo_guard/templates"
```
...
```text
Created repo-guard Python project: my-project
...
```python
# repo-guard: allow[type-ignore] reason="third-party package has broken stubs"
```
...
- `src/` exists.
- `tests/` exists.
- `docs/` exists unless docs are disabled in `repo-guard.toml`.
- `.github/workflows/ci.yml` exists unless CI is disabled in `repo-guard.toml`.
...
- `repo-guard.toml` selected agents match the files present.
- `coverage_fail_under` in `repo-guard.toml` matches `pyproject.toml` coverage config if present.
- `python_min` in `repo-guard.toml` matches generated `pyproject.toml`.
...
| `AGENTS.md`, `CLAUDE.md`, `.cursor/rules/**`, `.claude/rules/**` | run `repo-guard check`; require rule compilation if source-of-truth changes |
...
- Agent-rule diff requires `repo-guard check`.
- JSON output is deterministic.

---

## 6.4 `repo-guard validate`
...
Regenerate agent instruction files from templates and `repo-guard.toml`.
...
```markdown
<!-- generated by repo-guard; edit repo-guard.toml or rerun repo-guard compile-rules -->
```
...
name = "{{ project_slug }}"
version = "0.1.0"
description = "{{ project_slug }} generated by repo-guard."
readme = "README.md"
...
- Run `repo-guard inspect-diff` before final validation when possible.
- Run `repo-guard validate` before claiming completion.
...
- Instructions are guidance, not enforcement. Respect `repo-guard check` and CI as the source of truth for policy enforcement.
```
...
- `test_inspect_diff_agent_rules_requires_repo_guard_check`
...
1. Run `repo-guard init demo --agent all`.
2. Assert generated tree exists.
3. Run `repo-guard check --path demo`.
4. Optionally run:
   - `uv sync --all-groups`;
   - `uv run ruff format --check .`;
   - `uv run ruff check .`;
...
1. Generate project.
2. Run `repo-guard compile-rules`.
...
- [ ] Add `src/repo_guard/__init__.py`.
- [ ] Add `src/repo_guard/__main__.py`.
- [ ] Add `src/repo_guard/cli.py` with Typer app and placeholder commands.
- [ ] Add Ruff, mypy, Pyright, pytest, coverage config.
- [ ] Add initial tests for `repo-guard version`.
- [ ] Add CI for the CLI package itself.

### Acceptance criteria

- `uv run repo-guard version` works.
...
- [ ] Render `repo-guard.toml`.
- [ ] Add CLI output summary.

### Acceptance criteria

- `repo-guard init demo --agent codex` produces a valid package tree.
...
- `repo-guard check` passes in a freshly generated project.
...
- Agent rules changes require `repo-guard check`.
...
- `repo-guard doctor` reports useful environment status.
- `repo-guard validate --quick` runs without shell=True.
...
- Local wheel install exposes `repo-guard`.
...
1. `repo-guard init demo --agent all` creates a complete project.
2. The generated project passes `repo-guard check`.
...
5. `repo-guard inspect-diff` produces useful required-validation guidance.
...
- `repo-guard upgrade` to apply starter updates to existing generated repos
- richer `repo-guard.toml` diff rules
...
- Homebrew formula
- GitHub Action for `repo-guard check`
...
```text
Implement V1 of repo-guard according to docs/V1_IMPLEMENTATION_PLAN.md.
