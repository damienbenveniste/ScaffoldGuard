# V1 Implementation Plan: `agent-safe-python`

**Purpose:** Implementation-ready plan for a coding agent to build V1 of a PyPI-installable Python starter CLI that creates **agent-safe Python repositories** with first-class **Codex**, **Claude Code**, and **Cursor** support.

**Working package name:** `agent-safe-python`
**Working CLI command:** `agent-safe`
...
```bash
uvx agent-safe-python init my_project --agent codex
uvx agent-safe-python init my_project --agent claude
uvx agent-safe-python init my_project --agent cursor
...
```bash
agent-safe init NAME [options]
agent-safe check [options]
agent-safe inspect-diff [options]
agent-safe validate [options]
agent-safe compile-rules [options]
agent-safe doctor [options]
agent-safe version
```
...
```toml
[project]
name = "agent-safe-python"
version = "0.1.0"
description = "Generate agent-safe Python project starters for Codex, Claude Code, and Cursor."
...
packages = ["src/agent_safe"]

[tool.hatch.build.targets.wheel.force-include]
"src/agent_safe/templates" = "agent_safe/templates"
```
...
```text
Created agent-safe Python project: my-project
...
```python
# agent-safe: allow[type-ignore] reason="third-party package has broken stubs"
```
...
- `src/` exists.
- `tests/` exists.
- `docs/` exists unless docs are disabled in `agent-safe.toml`.
- `.github/workflows/ci.yml` exists unless CI is disabled in `agent-safe.toml`.
...
- `agent-safe.toml` selected agents match the files present.
- `coverage_fail_under` in `agent-safe.toml` matches `pyproject.toml` coverage config if present.
- `python_min` in `agent-safe.toml` matches generated `pyproject.toml`.
...
| `AGENTS.md`, `CLAUDE.md`, `.cursor/rules/**`, `.claude/rules/**` | run `agent-safe check`; require rule compilation if source-of-truth changes |
...
- Agent-rule diff requires `agent-safe check`.
- JSON output is deterministic.

---

## 6.4 `agent-safe validate`
...
Regenerate agent instruction files from templates and `agent-safe.toml`.
...
```markdown
<!-- generated by agent-safe-python; edit agent-safe.toml or rerun agent-safe compile-rules -->
```
...
name = "{{ project_slug }}"
version = "0.1.0"
description = "{{ project_slug }} generated by agent-safe-python."
readme = "README.md"
...
- Run `agent-safe inspect-diff` before final validation when possible.
- Run `agent-safe validate` before claiming completion.
...
- Instructions are guidance, not enforcement. Respect `agent-safe check` and CI as the source of truth for policy enforcement.
```
...
- `test_inspect_diff_agent_rules_requires_agent_safe_check`
...
1. Run `agent-safe init demo --agent all`.
2. Assert generated tree exists.
3. Run `agent-safe check --path demo`.
4. Optionally run:
   - `uv sync --all-groups`;
   - `uv run ruff format --check .`;
   - `uv run ruff check .`;
...
1. Generate project.
2. Run `agent-safe compile-rules`.
...
- [ ] Add `src/agent_safe/__init__.py`.
- [ ] Add `src/agent_safe/__main__.py`.
- [ ] Add `src/agent_safe/cli.py` with Typer app and placeholder commands.
- [ ] Add Ruff, mypy, Pyright, pytest, coverage config.
- [ ] Add initial tests for `agent-safe version`.
- [ ] Add CI for the CLI package itself.

### Acceptance criteria

- `uv run agent-safe version` works.
...
- [ ] Render `agent-safe.toml`.
- [ ] Add CLI output summary.

### Acceptance criteria

- `agent-safe init demo --agent codex` produces a valid package tree.
...
- `agent-safe check` passes in a freshly generated project.
...
- Agent rules changes require `agent-safe check`.
...
- `agent-safe doctor` reports useful environment status.
- `agent-safe validate --quick` runs without shell=True.
...
- Local wheel install exposes `agent-safe`.
...
1. `agent-safe init demo --agent all` creates a complete project.
2. The generated project passes `agent-safe check`.
...
5. `agent-safe inspect-diff` produces useful required-validation guidance.
...
- `agent-safe upgrade` to apply starter updates to existing generated repos
- richer `agent-safe.toml` diff rules
...
- Homebrew formula
- GitHub Action for `agent-safe check`
...
```text
Implement V1 of agent-safe-python according to docs/V1_IMPLEMENTATION_PLAN.md.
