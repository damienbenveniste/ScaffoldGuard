...
from agent_safe.checks.config import load_toml
from agent_safe.project_config import ProjectConfigError, load_generated_project_config
...
def _generated_project_checks(root: Path) -> tuple[DoctorCheck, ...]:
    ...
    checks = [
        DoctorCheck(
            ...
            ok=True,
            severity="info",
            message="agent-safe.toml parses.",
        ),
        DoctorCheck(
