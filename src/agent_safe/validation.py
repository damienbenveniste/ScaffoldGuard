...
from agent_safe.project_config import GeneratedProjectConfig, load_generated_project_config
...
def run_validation(path: Path, *, quick: bool, capture: bool) -> ValidationReport:
    ...
    for command in validation_commands(config, quick=quick):
        if command == ("agent-safe", "check"):
            status = _run_agent_safe_check(config.root)
        else:
...
def _run_agent_safe_check(root: Path) -> CommandStatus:
    """Run policy checks in process for stable CLI behavior."""
    try:
        report = run_checks(root)
    except CheckConfigurationError as exc:
        return CommandStatus(command=("agent-safe", "check"), exit_code=2, stderr=str(exc))
    if report.ok:
        return CommandStatus(command=("agent-safe", "check"), exit_code=0)
    return CommandStatus(
        command=("agent-safe", "check"),
        exit_code=1,
