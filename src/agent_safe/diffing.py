"""Diff inspection for generated agent-safe repositories."""
...
from agent_safe.checks.config import int_value, load_agent_safe_toml, str_value, table_value
...
def _apply_agent_rules(
    ...
    if any(_is_agent_rule_file(path) for path in files):
        validations.append("uv run agent-safe check")
